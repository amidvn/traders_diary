...
Процедура ОбработатьТинькоффOpenAPI()
	...
	Если Операции.Количество() > 0 Тогда
		СтруктураДляЗаголовков = Операции[0];
		Для Каждого ЭлементСтруктуры Из СтруктураДляЗаголовков Цикл
			ТаблЗнач.Колонки.Добавить(ЭлементСтруктуры.Ключ, , ЭлементСтруктуры.Ключ);
		КонецЦикла;
...
Функция ИнициироватьЗаголовки()
	...
	Токен = ""; // TODO: реализовать сохранение и считывание токена
	ДополнительныеПараметры.Вставить("Заголовки", Новый Структура("Authorization", "Bearer " + Токен));
