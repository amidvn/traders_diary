...
Процедура ОбработатьТинькоффPantini(ТабЗнач)
	...
	Для Каждого Стр Из ТабЗнач Цикл
		...
		ДатаВремяСделки = ОпределитьДатуВремяСделки(Стр[СоответствиеКолонок.Получить("date")], "ДФ=yyyy-MM-ddTHH:mm:ss");
...
Процедура ОбработатьВТБExcel(ТабЗнач)
	...
	Пока ЕстьЗаключенныеСделки Цикл
		...
		ДатаВремяСделки = ОпределитьДатуВремяСделки(Стр[СоответствиеКолонок.Получить("Дата сделки")], "ДФ='dd.MM.yyyy HH:mm:ss'");
