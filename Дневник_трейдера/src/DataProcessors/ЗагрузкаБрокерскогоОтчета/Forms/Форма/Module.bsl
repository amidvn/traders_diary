...
Процедура ПриОткрытии(Отказ)
	...
	ОпределитьПериодЗагрузки();
	ВариантПериодаЗагрузкиПриИзменении("");
КонецПроцедуры
...
Процедура СпособЗагрузкиБрокерскогоОтчетаПриИзменении(Элемент)
	...
	ИначеЕсли СпособЗагрузкиБрокерскогоОтчета = ПредопределенноеЗначение("Перечисление.СпособыЗагрузкиБрокерскогоОтчета.ТинькоффOpenAPI") Тогда 			
		ПериодЗагрузки = ОпределитьПериодЗагрузкиТинькоффOpenAPI(БрокерскийСчет);
	...
	ИзменениеНадписиУказаниеТокена();
	
КонецПроцедуры

&НаКлиенте
Процедура ВариантПериодаЗагрузкиПриИзменении(Элемент)
	Элементы.ПериодЗагрузки.Видимость = ВариантПериодаЗагрузки = 2;
КонецПроцедуры
...
Процедура Загрузить(Команда)
	...
	Если Элементы.ГруппаПериодЗагрузки.Видимость Тогда
		...
		Если Не ЗначениеЗаполнено(ПериодЗагрузки.ДатаОкончания) Тогда
			...
			Отказ = Истина;
		КонецЕсли;
		Если Не Отказ И ПериодЗагрузки.ДатаНачала > ПериодЗагрузки.ДатаОкончания Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = "Дата начала периода загрузки должна быть меньше даты окончания";
		    Сообщение.Поле = "ПериодЗагрузки";
		    Сообщение.Сообщить();
			Отказ = Истина;
		КонецЕсли;
...
Процедура УказатьТокенOpenApiТинькофф(Команда)
	...
	ОписаниеОповещенияОЗакрытии = Новый ОписаниеОповещения("УказатьТокенOpenApiТинькоффЗавершение", ЭтотОбъект);
	ОткрытьФорму(ИмяФормыДляВводаТокена,,ЭтаФорма,ЭтаФорма,,,ОписаниеОповещенияОЗакрытии, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры

&НаКлиенте
Процедура УказатьТокенOpenApiТинькоффЗавершение(Результат, ДопПараметры) Экспорт
	ИзменениеНадписиУказаниеТокена();
КонецПроцедуры
...
&НаСервере
Процедура ОпределитьПериодЗагрузки()

	Если СпособЗагрузкиБрокерскогоОтчета = Перечисления.СпособыЗагрузкиБрокерскогоОтчета.ТинькоффOpenAPI Тогда

		ТаблицаНастроек = ПолучитьТаблицуНастроек();
		НайдСтр = ТаблицаНастроек.Найти(БрокерскийСчет, "БрокерскийСчет");
		Если НайдСтр <> Неопределено Тогда
			Если НачалоДня(НайдСтр.ДатаПоследнейЗагрузки - 2*60*60) = НачалоДня(ТекущаяДатаСеанса()) Тогда
				ВариантПериодаЗагрузки = 0;
			ИначеЕсли НачалоДня(НайдСтр.ДатаПоследнейЗагрузки - 2*60*60) = НачалоДня(НачалоДня(ТекущаяДатаСеанса())-1) Тогда
				ВариантПериодаЗагрузки = 1;
			Иначе
				ВариантПериодаЗагрузки = 2;
				ПериодЗагрузки.ДатаНачала = НачалоДня(НайдСтр.ДатаПоследнейЗагрузки);
				ПериодЗагрузки.ДатаОкончания = КонецДня(ТекущаяДатаСеанса());
			КонецЕсли;
		Иначе
			ВариантПериодаЗагрузки = 2;
		КонецЕсли;
...
Процедура ЗагрузитьНаСервере()
	...
	ИначеЕсли СпособЗагрузкиБрокерскогоОтчета = Перечисления.СпособыЗагрузкиБрокерскогоОтчета.ВТБExcel Тогда
		ОбработатьВТБExcel(ТабЗнач);
	КонецЕсли;
...
Процедура ОбработатьТинькоффPantini(ТабЗнач)
	...
	Для Каждого Стр Из ТабЗнач Цикл
		...
		ДатаВремяСделки = ОпределитьДатуВремяСделки(Стр[СоответствиеКолонок.Получить("date")], "yyyy-MM-ddTHH:mm:ss");
		Если ТекВремяСделки <> ДатаВремяСделки Тогда
			СлужебныйПорядковыйНомер = 0;
			ТекВремяСделки = ДатаВремяСделки;
		Иначе
			СлужебныйПорядковыйНомер = СлужебныйПорядковыйНомер - 1;
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ТипСделки) Тогда
			...
			НовСтр.СлужебныйПорядковыйНомер = СлужебныйПорядковыйНомер;
			...
			НовСтр.Количество = Стр[СоответствиеКолонок.Получить("quantityExecuted")];
	...
	ТаблицаСделок.Сортировать("ДатаВремяСделки, СлужебныйПорядковыйНомер");
...
Процедура ОбработатьТинькоффOpenAPI()
	...
	Если ВариантПериодаЗагрузки = 0 Тогда
		ДатаНачалаЗагрузки = НачалоДня(ТекущаяДатаСеанса());
		ДатаОкончанияЗагрузки = КонецДня(ТекущаяДатаСеанса());
	ИначеЕсли ВариантПериодаЗагрузки = 1 Тогда
		ДатаНачалаЗагрузки = НачалоДня(НачалоДня(ТекущаяДатаСеанса())-1);
		ДатаОкончанияЗагрузки = КонецДня(ДатаНачалаЗагрузки);
	Иначе
		ДатаНачалаЗагрузки = ПериодЗагрузки.ДатаНачала;
		ДатаОкончанияЗагрузки = ПериодЗагрузки.ДатаОкончания;
	КонецЕсли;
	
	ПараметрыЗапроса = Новый Структура;
	ПараметрыЗапроса.Вставить("from", Формат(ДатаНачалаЗагрузки + 2*3600, "ДФ='yyyy-MM-ddTHH:mm:ss''+03:00';"));
	ПараметрыЗапроса.Вставить("to", Формат(ДатаОкончанияЗагрузки + 2*3600, "ДФ='yyyy-MM-ddTHH:mm:ss''+03:00';"));
	Если ЗначениеЗаполнено(БрокерскийСчет.НомерСчета) Тогда
		ПараметрыЗапроса.Вставить("brokerAccountId", БрокерскийСчет.НомерСчета);
	КонецЕсли;
	...
	// сохранить дату и время последней загрузки
	ТаблицаНастроек = ПолучитьТаблицуНастроек();
	
	НайдСтр = ТаблицаНастроек.Найти(БрокерскийСчет, "БрокерскийСчет");
	Если НайдСтр = Неопределено Тогда
		НайдСтр = ТаблицаНастроек.Добавить();
		НайдСтр.БрокерскийСчет = БрокерскийСчет;
	КонецЕсли;
	Если КонецДня(ПериодЗагрузки.ДатаОкончания) >= КонецДня(ТекущаяДатаСеанса()) Тогда
		НайдСтр.ДатаПоследнейЗагрузки = ТекущаяДатаСеанса();
	Иначе
		НайдСтр.ДатаПоследнейЗагрузки = ПериодЗагрузки.ДатаОкончания;
	КонецЕсли;
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("ЗагрузкаБрокерскогоОтчета", "ТаблицаНастроек", ТаблицаНастроек);
...
Функция ПолучитьТаблицуНастроек()
	...
	ТаблицаНастроек = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("ЗагрузкаБрокерскогоОтчета", "ТаблицаНастроек");
	Если ТаблицаНастроек = Неопределено Тогда
		ТаблицаНастроек = Новый ТаблицаЗначений();
		ТаблицаНастроек.Колонки.Добавить("БрокерскийСчет", Новый ОписаниеТипов("СправочникСсылка.БрокерскиеСчета"));
		ТаблицаНастроек.Колонки.Добавить("ДатаПоследнейЗагрузки", Новый ОписаниеТипов("Дата"));
	КонецЕсли;
...
Функция ПолучитьТаблицуСделок()
	...
	ТаблицаСделок.Колонки.Добавить("СлужебныйПорядковыйНомер");
...
&НаСервереБезКонтекста
Функция ОпределитьДатуВремяСделки(СтрокаДатаВремя, ФорматДаты)

	Ошибка = Ложь;
    Попытка 
        ПереданПравильныйФормат = Формат('00010101', "ДФ=" + ФорматДаты); 
    Исключение 
        Ошибка = Истина; 
        Возврат Дата('00010101'); 
    КонецПопытки;
    
    СтрокаДатаВремя = СтрЗаменить(СтрокаДатаВремя, "д", "d");
    СтрокаДатаВремя = СтрЗаменить(СтрокаДатаВремя, "М", "M");
    СтрокаДатаВремя = СтрЗаменить(СтрокаДатаВремя, "г", "y");
    СтрокаДатаВремя = СтрЗаменить(СтрокаДатаВремя, "ч", "h");
    СтрокаДатаВремя = СтрЗаменить(СтрокаДатаВремя, "Ч", "H");
    СтрокаДатаВремя = СтрЗаменить(СтрокаДатаВремя, "м", "m");
    СтрокаДатаВремя = СтрЗаменить(СтрокаДатаВремя, "с", "s");
     
    ЧДВ = Новый Соответствие; 
    Для Инд = 1 По СтрДлина(ФорматДаты) + 7 Цикл 
        ЧДВ[Сред(ФорматДаты + "dMyHhms", Инд, 1)] = 0    // - инициализация частей даты 
    КонецЦикла; 
    Для Инд = 1 По 12 Цикл  // - замена названий месяцев числами
        СтрокаДатаВремя = СтрЗаменить(СтрокаДатаВремя, Формат(Дата(1, Инд, 1), "ДФ=MMММ"), Формат(Инд, "ЧЦ=4; ЧВН=")); 
        СтрокаДатаВремя = СтрЗаменить(СтрокаДатаВремя, Формат(Дата(1, Инд, 1), "ДФ=MMМ" ), Формат(Инд, "ЧЦ=3; ЧВН=")); 
    КонецЦикла; 
    Для Инд = 1 По СтрДлина(ФорматДаты) Цикл 
        ЧДВ[Сред(ФорматДаты, Инд, 1)] = 10 * ЧДВ[Сред(ФорматДаты, Инд, 1)] + Найти("123456789", Сред(СтрокаДатаВремя, Инд, 1)); // накопление частей даты 
        Ошибка = Ошибка ИЛИ Найти("dMyHhms", Сред(ФорматДаты, Инд, 1)) И НЕ Найти("0123456789", Сред(СтрокаДатаВремя, Инд, 1)); // необязательная проверка на цифры 
    КонецЦикла; 
    ЧДВ["y"] = ЧДВ["y"] + ?(ЧДВ["y"] < 50, 2000, ?(ЧДВ["y"] < 100, 1900, 0)); // дополнение двух цифр года до четырех 
    Попытка 
        Возврат Дата(ЧДВ["y"], ЧДВ["M"], ЧДВ["d"], ЧДВ["H"] + ЧДВ["h"], ЧДВ["m"], ЧДВ["s"]) 
    Исключение 
        Ошибка = Истина; 
        Возврат Дата('00010101'); 
    КонецПопытки
	
	// образец строки СтрокаДатаВремя - "2021-01-08T21:06:06.728+03:00" "ДФ=yyyy-MM-ddTHH:mm:ss"
...
Функция ОпределитьЦеннуюБумагу(Тикер = "", Название, КодЦеннойБумаги, ТипИнструмента, Валюта)
	...
	ЦеннаяБумага = Справочники.ЦенныеБумаги.ПустаяСсылка();
	
	Если ЗначениеЗаполнено(Тикер) Тогда
		ЦеннаяБумага = Справочники.ЦенныеБумаги.НайтиПоНаименованию(СокрЛП(Тикер), Истина);
	ИначеЕсли ЗначениеЗаполнено(КодЦеннойБумаги) Тогда
		ЦеннаяБумага = Справочники.ЦенныеБумаги.НайтиПоРеквизиту("КодЦеннойБумаги", КодЦеннойБумаги);
		Если ЦеннаяБумага.Пустая() Тогда 
			Тикер = ОпределитьТикерПоКодуЦеннойБумаги(КодЦеннойБумаги);
		КонецЕсли;	
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция ОпределитьТикерПоКодуЦеннойБумаги(КодЦеннойБумаги)
...
&НаСервере
Процедура СоздатьОбновитьДокументыСделкиЗаДень(ТаблицаСделок)
...
Процедура ПодготовитьВнутридневныеСделки(СделкиЗаДень, ДатаДень, БрокерскийСчет)
	...
	ТаблицаВнутридневныхСделок.Колонки.Добавить("НаправлениеСделки");
	...
	ТаблицаЦенныхБумаг = СделкиЗаДень.Скопировать(, "ЦеннаяБумага");
	ТаблицаЦенныхБумаг.Свернуть("ЦеннаяБумага");
	МассивЦенныхБумаг = ТаблицаЦенныхБумаг.ВыгрузитьКолонку("ЦеннаяБумага");
	ОстаткиНаНачалоДня = ПолучитьОстаткиНаНачалоДня(БрокерскийСчет, ДатаДень + 2*60*60, МассивЦенныхБумаг); // остатки берутся на 2 ночи (после окончания торговой сессии на СПб-бирже
	
	Для Каждого Стр Из СделкиЗаДень Цикл
		
		ТекКоличество = Стр.Количество;
		
		СтруктураПоиска = Новый Структура("ЦеннаяБумага, Валюта", Стр.ЦеннаяБумага, Стр.Валюта);
		
		МассивНайдСтрокВОстатках = ОстаткиНаНачалоДня.НайтиСтроки(СтруктураПоиска); 
		Если МассивНайдСтрокВОстатках.Количество() <> 0 И МассивНайдСтрокВОстатках[0].КоличествоОстаток <> 0 Тогда
			НайдСтрокаОстатки = МассивНайдСтрокВОстатках[0];
			НачальноеКоличество = НайдСтрокаОстатки.КоличествоОстаток;
			Если Стр.ТипСделки = Перечисления.ТипыСделок.Покупка Тогда
				НайдСтрокаОстатки.КоличествоОстаток = НайдСтрокаОстатки.КоличествоОстаток + Мин(-НайдСтрокаОстатки.КоличествоОстаток, ТекКоличество);
				ТекКоличество = ТекКоличество - (НайдСтрокаОстатки.КоличествоОстаток - НачальноеКоличество);
			ИначеЕсли Стр.ТипСделки = Перечисления.ТипыСделок.Продажа Тогда
				НайдСтрокаОстатки.КоличествоОстаток = НайдСтрокаОстатки.КоличествоОстаток - Мин(НайдСтрокаОстатки.КоличествоОстаток, ТекКоличество);
				ТекКоличество = ТекКоличество - (НачальноеКоличество - НайдСтрокаОстатки.КоличествоОстаток);
			КонецЕсли;		
		КонецЕсли;
		...
		Если МассивНайдСтрок.Количество() = 0 Тогда
			...
			НайдСтрока.НаправлениеСделки = ?(Стр.ТипСделки = Перечисления.ТипыСделок.Покупка, 
				Перечисления.НаправленияСделки.Лонг, Перечисления.НаправленияСделки.Шорт);
		Иначе
...
&НаСервереБезКонтекста
Функция ПолучитьОстаткиНаНачалоДня(БрокерскийСчет, Период, МассивЦенныхБумаг)

	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	|	ОстаткиЦенныхБумагОстатки.ЦеннаяБумага,
	|	ОстаткиЦенныхБумагОстатки.Валюта,
	|	ОстаткиЦенныхБумагОстатки.КоличествоОстаток
	|ИЗ
	|	РегистрНакопления.ОстаткиЦенныхБумаг.Остатки(&Период, БрокерскийСчет = &БрокерскийСчет
...
&НаКлиенте
Процедура ИзменениеНадписиУказаниеТокена()
	
	Если Не СпособЗагрузкиБрокерскогоОтчета = ПредопределенноеЗначение("Перечисление.СпособыЗагрузкиБрокерскогоОтчета.ТинькоффOpenAPI") Тогда
		Возврат;
	КонецЕсли;
	
	Если ТокенOpenApiТинькоффУказан() Тогда
		Элементы.УказатьТокенOpenApiТинькофф.Заголовок = "Токен указан";
	Иначе	
		Элементы.УказатьТокенOpenApiТинькофф.Заголовок = "Указать токен";
	КонецЕсли;	
...
Функция ИнициироватьЗаголовки()
	...
	ТокенOpenApiТинькофф = ПолучитьТокенOpenApiТинькофф();
...
&НаСервереБезКонтекста
Функция ПолучитьТокенOpenApiТинькофф()
	Возврат ОбщегоНазначения.ПрочитатьДанныеИзБезопасногоХранилища("ТокенOpenAPIТинькофф", "Токен");
КонецФункции	

&НаСервереБезКонтекста
Функция ТокенOpenApiТинькоффУказан()
	Возврат ЗначениеЗаполнено(ПолучитьТокенOpenApiТинькофф());
КонецФункции	

&НаСервереБезКонтекста
Функция ОпределитьПериодЗагрузкиТинькоффOpenAPI(БрокерскийСчет)
	
	ДатаНачала = НачалоДня(ТекущаяДатаСеанса() - 7 * 86400);
	
	Запрос = Новый Запрос(
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	СделкиЗаДень.Дата КАК Дата
	|ИЗ
	|	Документ.СделкиЗаДень КАК СделкиЗаДень
	|ГДЕ
	|	СделкиЗаДень.Проведен
	|	И СделкиЗаДень.БрокерскийСчет = &БрокерскийСчет
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ ПЕРВЫЕ 1
	|	ДвижениеДенежныхСредств.Дата
	|ИЗ
	|	Документ.ДвижениеДенежныхСредств КАК ДвижениеДенежныхСредств
	|ГДЕ
	|	ДвижениеДенежныхСредств.Проведен
	|	И ДвижениеДенежныхСредств.БрокерскийСчет = &БрокерскийСчет
	|УПОРЯДОЧИТЬ ПО
	|	Дата УБЫВ");
	...
	Если Выборка.Следующий() Тогда
		ДатаНачала = НачалоДня(Выборка.Дата + 86400);
	КонецЕсли;
...
&НаСервере
Процедура ОбработатьВТБExcel(ТабЗнач)
	
	// поиск блока "Заключенные в отчетном периоде сделки с ценными бумагами"
	НачалоБлокаЗаключенныеСделки = 0;	
	Для Инд = 0 По ТабЗнач.Количество()-1 Цикл
		Если ТабЗнач[Инд]["К_1"] = "Заключенные в отчетном периоде сделки с ценными бумагами" 
			 ИЛИ ТабЗнач[Инд]["К_2"] = "Заключенные в отчетном периоде сделки с ценными бумагами" Тогда
			НачалоБлокаЗаключенныеСделки = Инд;
			Прервать;
		КонецЕсли;
	КонецЦикла;		
	
	Если НачалоБлокаЗаключенныеСделки = 0 Тогда
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = "Загрузка не выполнена: в указанном файле не найден блок ""Заключенные в отчетном периоде сделки с ценными бумагами""";
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
	...
	СтрокаСКолонками = ТабЗнач[НачалоБлокаЗаключенныеСделки+1]; // ТабЗнач[339]  ТабЗнач[340]
	
	СоответствиеКолонок = Новый Соответствие;
	Для Каждого Колонка Из ТабЗнач.Колонки Цикл
		ИмяКолонки = Колонка.Имя;
		Если ТипЗнч(СтрокаСКолонками[ИмяКолонки]) <> Тип("Строка") Тогда
			Продолжить;
		КонецЕсли;
		Если СтрНайти(СтрокаСКолонками[ИмяКолонки], "Наименование ценной бумаги") > 0 Тогда
			СоответствиеКолонок.Вставить("Наименование", ИмяКолонки);
		ИначеЕсли СтрокаСКолонками[ИмяКолонки] = "Дата и время заключения сделки" Тогда	
			СоответствиеКолонок.Вставить("Дата сделки", ИмяКолонки);
		ИначеЕсли СтрокаСКолонками[ИмяКолонки] = "Вид сделки" Тогда	
			СоответствиеКолонок.Вставить("Вид сделки", ИмяКолонки);
		ИначеЕсли СтрНайти(СтрокаСКолонками[ИмяКолонки], "Количество") > 0 Тогда	
			СоответствиеКолонок.Вставить("Количество", ИмяКолонки);
		ИначеЕсли СтрНайти(СтрокаСКолонками[ИмяКолонки], "Валюта расчетов") > 0 Тогда	
			СоответствиеКолонок.Вставить("Валюта", ИмяКолонки);
		ИначеЕсли СтрНайти(СтрокаСКолонками[ИмяКолонки], "Цена") > 0 Тогда	
			СоответствиеКолонок.Вставить("Цена", ИмяКолонки);
		ИначеЕсли СтрНайти(СтрокаСКолонками[ИмяКолонки], "Сумма сделки") > 0 Тогда	
			СоответствиеКолонок.Вставить("Сумма сделки", ИмяКолонки);
		ИначеЕсли СтрНайти(СтрокаСКолонками[ИмяКолонки], "НКД") > 0 Тогда	
			СоответствиеКолонок.Вставить("НКД", ИмяКолонки);
		ИначеЕсли СтрНайти(СтрокаСКолонками[ИмяКолонки], "Комиссия Банка за расчет по сделке") > 0 Тогда	
			СоответствиеКолонок.Вставить("Комиссия биржи", ИмяКолонки);
		ИначеЕсли СтрНайти(СтрокаСКолонками[ИмяКолонки], "Комиссия Банка за заключение сделки") > 0 Тогда	
			СоответствиеКолонок.Вставить("Комиссия брокера", ИмяКолонки);
		ИначеЕсли СтрокаСКолонками[ИмяКолонки] = "№ сделки" Тогда	
			СоответствиеКолонок.Вставить("Номер сделки", ИмяКолонки);
		КонецЕсли;	
	КонецЦикла;
	
	// проверка наличия всех колонок
	МассивОтсутствующихКолонок = Новый Массив;
	НеобходимыеКолонки = "Наименование,Дата сделки,Вид сделки,Количество,Валюта,Цена,Сумма сделки,НКД,Комиссия биржи,Комиссия брокера,Номер сделки";
	МассивНеобходимыхКолонок = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(НеобходимыеКолонки, ",");
	Для Каждого НеобходимаяКолонка Из МассивНеобходимыхКолонок Цикл
		Если СоответствиеКолонок.Получить(НеобходимаяКолонка) = Неопределено Тогда
			МассивОтсутствующихКолонок.Добавить(НеобходимаяКолонка);
		КонецЕсли;
	КонецЦикла;
	Если МассивОтсутствующихКолонок.Количество() > 0 Тогда
		Сообщение = Новый СообщениеПользователю();
		Сообщение.Текст = "Загрузка не выполнена: в указанном файле не найдены все необходимые колонки: " + СтрСоединить(МассивОтсутствующихКолонок, ",");
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;		
	
	КолонкаНаименование = СоответствиеКолонок.Получить("Наименование");
	Инд = НачалоБлокаЗаключенныеСделки + 2;
	ЕстьЗаключенныеСделки = ЗначениеЗаполнено(ТабЗнач[Инд][КолонкаНаименование]);
	Пока ЕстьЗаключенныеСделки Цикл
		
		Стр = ТабЗнач[Инд];
		
		ТипСделки = Перечисления.ТипыСделок.ПустаяСсылка();
		Если Стр[СоответствиеКолонок.Получить("Вид сделки")] = "Покупка" Тогда
			ТипСделки = Перечисления.ТипыСделок.Покупка;
		ИначеЕсли Стр[СоответствиеКолонок.Получить("Вид сделки")] = "Продажа" Тогда
			ТипСделки = Перечисления.ТипыСделок.Продажа;
		КонецЕсли;	
		
		Валюта = ОпределитьВалюту(Стр[СоответствиеКолонок.Получить("Валюта")]);
		ДатаВремяСделки = ОпределитьДатуВремяСделки(Стр[СоответствиеКолонок.Получить("Дата сделки")], "dd.MM.yyyy HH:mm:ss");
		
		//Тикер = Стр[СоответствиеКолонок.Получить("ticker")];
		//Название = Стр[СоответствиеКолонок.Получить("showName")];
		МассивЭлементовНаименования = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(Стр[КолонкаНаименование], ",");
		НаименованиеБумаги = МассивЭлементовНаименования[0];
		ISIN = "";
		Если МассивЭлементовНаименования.Количество() >= 3 Тогда
			ISIN = МассивЭлементовНаименования[2];
		КонецЕсли;
		ОписаниеОперации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1 %2 штук %3", 
			Стр[СоответствиеКолонок.Получить("Вид сделки")],
			Стр[СоответствиеКолонок.Получить("Количество")],
			НаименованиеБумаги); 
		Если СтрНайти(НаименованиеБумаги, "ETF") Тогда
			ТипИнструмента = Перечисления.ТипыИнструментов.ETF;
		ИначеЕсли Стр[СоответствиеКолонок.Получить("НКД")] > 0 Тогда
			ТипИнструмента = Перечисления.ТипыИнструментов.Облигация;
		Иначе
			ТипИнструмента = Перечисления.ТипыИнструментов.Акция;
		КонецЕсли;
		...
		НовСтр = ТаблицаСделок.Добавить();
		НовСтр.ИдентификаторСделки = Формат(Стр[СоответствиеКолонок.Получить("Номер сделки")], "ЧГ=0");
		НовСтр.ДатаДень = НачалоДня(ДатаВремяСделки - 2*60*60); // сделки до двух часов ночи относятся к сессии предыдущего дня
		НовСтр.ДатаВремяСделки = ДатаВремяСделки;
		НовСтр.ЦеннаяБумага = ОпределитьЦеннуюБумагу(, НаименованиеБумаги, ISIN, ТипИнструмента, Валюта);
		НовСтр.Валюта = Валюта;
		НовСтр.ТипСделки = ТипСделки;
		НовСтр.Количество = Стр[СоответствиеКолонок.Получить("Количество")];
		НовСтр.Цена = Стр[СоответствиеКолонок.Получить("Цена")];
		НовСтр.Сумма = Стр[СоответствиеКолонок.Получить("Сумма сделки")];
		НовСтр.Комиссия = Стр[СоответствиеКолонок.Получить("Комиссия биржи")] + Стр[СоответствиеКолонок.Получить("Комиссия брокера")];
		НовСтр.Описание = ОписаниеОперации;
		
		Инд = Инд + 1;
		ЕстьЗаключенныеСделки = Инд < ТабЗнач.Количество() И ЗначениеЗаполнено(Стр[КолонкаНаименование]);
